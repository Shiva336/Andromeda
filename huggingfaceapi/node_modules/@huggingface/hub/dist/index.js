...
async function sha256Node(buffer) {
  ...
  const transform256 = (buffer instanceof Blob ? import_node_stream.Readable.fromWeb(buffer.stream()) : import_node_stream.Readable.from(Buffer.from(buffer))).pipe(sha256Stream).setEncoding("hex");
  return promisifyRs(transform256);
}
var import_node_stream, import_node_crypto, promisifyRs;
...
async function* commitIter(params) {
  ...
  for (const operations of chunk(allOperations.filter(isFileOperation), 100)) {
    ...
    const res2 = await (params.fetch ?? fetch)(
        ...
        if (header?.chunk_size) {
          ...
          await promisesQueueStreaming(
            parts.map((part) => async () => {
              ...
              const res4 = await (params.fetch ?? fetch)(header[part], {
                method: "PUT",
          ...
          const res3 = await (params.fetch ?? fetch)(completionUrl, {
            method: "POST",
          ...
          const res3 = await (params.fetch ?? fetch)(obj.actions.upload.href, {
            method: "PUT",
            headers: {
...
async function* listSpaces(params) {
  checkCredentials(params?.credentials);
  const search = new URLSearchParams([
    ...Object.entries({ limit: "500", ...params?.search?.owner ? { author: params.search.owner } : void 0 }),
